...
contract Helper {
    ...
    /**
     * @dev get aave mkr uniswap market ropsten address 
     */
    function getMkr() public pure returns (address mkr) {
        mkr = 0x2709bca0Ac821dA5E7F649544F70F95E574898F1;
    }
     /**
    ...
    /**
     * @dev get uni mkr-eth uniswap market ropsten address 
     */
    function getExchange() public pure returns (address exchange) {
        exchange = 0xed4597DCd234867d7A260AD24bAb8253F64940a5;
    }
     /**
     * @dev get uni dai-eth uniswap market ropsten address 
     */
    function getdaiExchange() public pure returns (address daiexchange) {
        daiexchange = 0xC245A7d35E652Cae438A1FdB13E474DF53DBB81D;
    }
}
    ...
    function zappify(uint256 maxAmount) public payable returns(bool)
        {
            ...
            // Approving The exchange and lending pool to spent token
            IERC20(getMkr()).approve(getExchange(), maxAmount);
            IERC20(getDai()).approve(getdaiExchange(), maxAmount);

            
            // ETH To Token Swap
            uint256 TokenAmt = UniswapInterface(getExchange()).ethToTokenSwapInput.value(ethToSwap)(minAmt, block.timestamp + 300);
            
            // Adding Liquidity
            uint256 uniToken = UniswapInterface(getExchange()).addLiquidity.value(ethToSwap)(minAmt, TokenAmt, block.timestamp + 300);
            ...
            AaveInterface(getLendingPool()).deposit.value(0)(getExchange(), uniToken, 0);
            
            // Loan To Value Ratio for borrowing dai is 75 % so for safety keeping it as 70 %
            uint256 daiToBorrow = (uniToken.mul(70)).div(100);
            
            //Borrowing Safe Amount of DAI
            AaveInterface(getLendingPool()).borrow(getDai(), daiToBorrow, 1, 0);
            
            // swap 100% dai borrowed to ETH Swap
            uint256 ethAmount = UniswapInterface(getdaiExchange()).tokenToEthSwapInput(daiToBorrow, 10, block.timestamp + 300);
            
            // for swapping half ETH we got back to token
            uint256 ethamt =  ethAmount.div(2);
            
            // swap 50% ETH amount to token 
            uint256 tokAmt = UniswapInterface(getExchange()).ethToTokenSwapInput.value(ethamt)(minAmt, block.timestamp + 300);
            tokAmt = tokAmt.mul(100);
            // Adding Liquidity
            uniToken =  UniswapInterface(getExchange()).addLiquidity.value(ethamt)(minAmt, tokAmt, block.timestamp + 300);
